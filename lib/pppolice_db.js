class UserGotBanned extends Error {
  constructor(suspect) {
    super(`User ${suspect.id}: ${suspect.name} got banned.`); // (1)
    this.user = suspect;
...
function pppolice(name, mode = 0) {
  ...
  const db = new require("./database")(
    tools.getCollectionNameByGameId(config.mode)
  );
  ...
  async function readBP(suspect) {
    if (verboseLevel >= 5)
      report.emit("report.verbose", `Attempt to read bp of ${suspect.id}`);
    try {
      let monitor = new osu.Api(policeId(), { parseNumeric: true });
      ...
      try {
        const user = await db.getUserApi(suspect.id);
        suspect.status = "normal";
        suspect.lastUpdate = new Date().getTime();
        await db.insertOrUpdateUser(suspect);
        return [];
      } catch (Error) {
        console.log(Error);
  ...
  async function checkBPppChange(account, newbp) {
    if (account.bp === [] || account.bp === undefined) {
      ...
      let result = await Promise.all(
        account.bp.map(async (score) => {
          let newScore = newbp.find(
            (newScore) => score.beatmapId == newScore.beatmapId
          );
  ...
  async function checkNewBP(account, newbp) {
    let result = await Promise.all(
      newbp.map(async (score) => {
        if (
          undefined ==
          account.bp.find((lastbp) => lastbp.beatmapId == score.beatmapId)
        ) {
          //new score
          let beatmapSet = await readBeatmap({ b: score.beatmapId });
          let thismap = beatmapSet[0];
  ...
  async function saveBP(account, newbp) {
    if (verboseLevel >= 5)
      report.emit("report.verbose", `saving bp to ${account.id}`);
    ...
    await Promise.all(
      newbp.map(async (score) => {
        let oldScoreIndex = account.bp.findIndex(
          (lastbp) => lastbp.beatmapId == score.beatmapId
        );
        if (-1 == oldScoreIndex) {
          needSave = true;
  ...
  async function manualProcessPlayerScore({ u, b, m = 0 }, save = true) {
    ...
    if (scoreSet !== [] && scoreSet !== undefined) {
      await Promise.all([
        checkNewBP(u, scoreSet),
        checkBPppChange(u, scoreSet),
      ]);
      if (save) {
        saveBP(u, scoreSet);
      }
  ...
  async function updatePlayer(account) {
    try {
      ...
      if (account.bp === [] || account.bp === undefined) {
        ...
        await Promise.all([
          checkNewBP(account, bp),
          checkBPppChange(account, bp),
        ]);
        account.lastUpdate = new Date().getTime();
        // await saveBP(account, bp);
        // update new bp should be handled by event
        delete account.bp;
        await db.insertOrUpdateUser(account);
      }
      return true;
  ...
  async function patrol() {
    if (verboseLevel >= 3) report.emit("report.verbose", "patrolling...");
    Promise.all(
      (await db.all({ fields: db.fields.handleStatus })).map(
        async (account) => {
          // update banned players every day in case he/she got unbanned
          if (account.status == "banned") {
            if (new Date().getTime() - account.lastUpdate > 1000 * 60 * 60 * 24)
              setTimeout((_) => {
                updatePlayer(account);
              }, (account.id % config.intervals.update) * 1000);
          }
          //account here is the key of suspects array.
          else if (account.lastUpdate !== undefined) {
            if (
              new Date().getTime() - account.lastUpdate >
              config.intervals.update * 2 * 1000
            )
              setTimeout((_) => {
                updatePlayer(account);
              }, (account.id % config.intervals.update) * 1000);
