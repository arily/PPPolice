    ...
    async function patrol() {
        ...
        Promise.all((await db.all()).map(async account => {
            ...
            if (account.lastUpdate !== undefined) {
                if (new Date().getTime() - account.lastUpdate > 5 * 60 * 1000) setTimeout( _ =>{updatePlayer(account)}, account.id % 300 * 1000) ;
            } else {
                setTimeout( _ =>{updatePlayer(account)}, account.id % 300 * 1000)
            }
