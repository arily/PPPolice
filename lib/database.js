function database(mode = 'osu') {
    ...
    async function getUserApi(u, field = undefined, updateDatabase = true) {
        try {
            ...
            if (updateDatabase) insertOrUpdateUser(account);
            let ret = {};
            if (field !== undefined) {
                let { fields } = field;
    ...
    //     let [doc, header] = await db.collection(gameMode).fetchAsync(selector);
    //     rebindProto(doc);
    //     return doc;
    // }
    async function findOne(selector, params) {
        ...
        let result = await db.collection(gameMode).findOne(selector, params);
    ...
    async function find(selector, params) {
        ...
        let result = await db.collection(gameMode).find(selector, params).toArray();
    ...
    async function deleteBadScores(){
        ...
        for (let user of users) {
            //let apiResult = await getUserApi(user.id);
            //apiResult.lastUpdate = new Date().getTime();
            let changed = user.bp.find(bp => bp.rank == null);
            if (changed) {
                console.log('found bad score.');
                let bad = user.bp.filter(bp => {
                    let bad = (bp.rank == null);
                    if (bad){
                ...
                let good = user.bp.filter(bp => !(bp.rank == null));
                user.bp = good;
