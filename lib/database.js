...
function database(mode = 'osu') {
    ...
    async function getUserApi(u, field = undefined, updateDatabase = true) {
        ...
        if (updateDatabase) {
            await insertOrUpdateUser(account);
        }
        let ret = {};
        if (field !== undefined) {
            let { fields } = field;
    ...
    async function saveDeleted(account, oldScore) {
        let { client, db } = await getDb();
        db.collection('removed').insertOne({
            account,
            mode: gameMode,
            score: oldScore
        })
    }
    async function saveOne(type, { beatmap, account, newScore }) {
        let { client, db } = await getDb();
        const collection = db.collection(gameMode)
        // const removed = db.collection("removed")
        if (['report.defarm', 'report.refarm'].includes(type)) {
            const old = await collection.find(
                { "id": account.id },
                {
                    projection: {
                        "bp": { $elemMatch: { beatmapId: newScore.beatmapId } },
                    },
                }
            ).toArray()
            // console.log(old)
            try{
                const oldScore = old[0].bp[0];
                // console.log(oldScore)
                this.saveDeleted(account, oldScore)
                const result = await collection.updateOne(
                    { "id": account.id },
                    { $pull: { bp: { beatmapId: newScore.beatmapId } } },
                )
            } catch (error){
                console.log(error);
            }
            // console.log(result.toString())
        } else if (!['report.farm'].includes(type)) {
    ...
    async function readBeatmapFresh(beatmap) {
        ...
        try {
            const rtn = await monitor.getBeatmaps(beatmap);
            return rtn;
        } catch (e) {
            return readBeatmapFresh(beatmap);
        }
    }
