function match({ teams, mappool, rules }) {
    ...
    this.pickPlay = function(picker, vol = 1) {
        ...
        for (let i = 0; i < vol; i++) {
            if (team.red == 5 && team.blue == 5) {
                ...
                this.console.log(`${this.name(picker)} pick ${pick1}`);
            ...
            this.console.log("result:", str);
            lastwin = this.last_win();
            team[lastwin] += 1;
            this.console.log(`${this.name('red')} ${team.red}:${team.blue} ${this.name('blue')}`);
            events[events.length] = {
                type: 'judge',
                result: {
                    winner: lastwin,
    ...
    this.ban = function(picker1, vol = 1) {
        ...
        for (let i = 0; i < vol; i++) {
            ...
            this.console.log(`${this.name(picker1)} ban ${banned}`);
            events[events.length] = {
            	type: 'ban',
            	result: {
    ...
    this.roll = function() {
        ...
        this.console.log(`${this.name('red')} rolls ${cfirstroll}, ${this.name('blue')} rolls ${hfirstroll}`);

        if (cfirstroll == hfirstroll) {
            this.console.log('Again. ');
            return this.roll();
        }
    ...
    this.matchInstruction = function* (out = true) {
        ...
        rules.instruction.forEach(instruction => {
            ...
            if (instruct == 'roll') {
                ...
                yield  (this[instruct](picker1, ...instruction));
                yield  (this[instruct](picker2, ...instruction));
            }
        });
        return events;
    }
    this.step = function*(){
    	...
    	let steps = this.matchInstruction(rules.instruction);
